...
<body>
  ...
  function zoneFileName(domain) {
    const safeDomain = String(domain || 'domain')
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9.-]+/g, '-')
      .replace(/^-+|-+$/g, '') || 'domain';
    return `${safeDomain}.zone`;
  }

  function zoneEscapeTxt(value) {
    return `"${String(value).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`;
  }

  function zoneTarget(value) {
    const target = String(value || '').trim();
    if (!target || target.endsWith('.')) return target;
    return `${target}.`;
  }

  function zoneName(name) {
    const recordName = String(name || '').trim();
    if (!recordName || recordName === '@') return '@';
    return recordName;
  }

  function zoneRecordLine(row) {
    const name = zoneName(row.name).padEnd(28, ' ');
    const ttl = String(row.ttl || DEFAULT_TTL).padEnd(6, ' ');
    const type = String(row.type || '').toUpperCase().padEnd(5, ' ');

    if (row.type === 'TXT') {
      return `${name} ${ttl} IN ${type} ${zoneEscapeTxt(row.value)}`;
    }

    if (row.type === 'CNAME') {
      return `${name} ${ttl} IN ${type} ${zoneTarget(row.value)}`;
    }

    if (row.type === 'MX') {
      const parts = String(row.value || '').trim().split(/\s+/);
      const priority = parts.shift() || '0';
      return `${name} ${ttl} IN ${type} ${priority} ${zoneTarget(parts.join(' '))}`;
    }

    return `${name} ${ttl} IN ${type} ${row.value}`;
  }

  function exportToZone(out) {
    const lines = [
      `; Microsoft 365 DNS zone export`,
      `; Domain: ${out.customDomain}`,
      `; Generated by Microsoft 365 Records Generator`,
      `; Records with * should be checked in your Microsoft 365 tenant before publishing.`,
      `$ORIGIN ${zoneTarget(out.customDomain)}`,
      `$TTL ${DEFAULT_TTL}`,
      ''
    ];

    out.recordOrder.forEach(section => {
      const records = out.records[section];
      if (!records?.length) return;

      lines.push(`; ${section}${section.includes('*') ? ' - check this record in your tenant' : ''}`);
      records.forEach(record => {
        lines.push(zoneRecordLine({
          section,
          ttl: record.ttl || DEFAULT_TTL,
          type: record.type,
          name: record.name,
    ...
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = zoneFileName(out.customDomain);
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
    URL.revokeObjectURL(url);
