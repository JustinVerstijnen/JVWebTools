...
function addInlineBlockControls() {
  els.visualEditor.querySelectorAll(".block-remove-button").forEach(button => button.remove());
  els.visualEditor.querySelectorAll(".editable-card, figure.image-block, table").forEach(block => {
...
function setView(view) {
  ...
  if (view === "editor") {
    ...
    if (markdownValue && els.markdownEditor.style.display === "block") els.visualEditor.innerHTML = markdownToHtml(markdownValue);
    els.visualEditor.style.display = "block";
    els.editorToolbar.style.display = "flex";
    els.markdownEditor.style.display = "none";
    normalizeEditorContent();
  } else {
    els.markdownEditor.value = buildMarkdown();
...
function insertImageFromPanel() {
  ...
  const alt = els.imageAlt.value.trim();
  if (!url) { showToast("Paste an image URL first"); return; }
  els.imagePanel.classList.remove("open");
  insertHtmlAtCursor(imageHtml(url, alt || "Image", alt || "Optional caption"), pendingPanelInsertOptions);
...
function insertTableFromPanel() {
  ...
  insertHtmlAtCursor(tableHtml(els.tableColumns.value, els.tableRows.value, els.tableAlign.value), pendingPanelInsertOptions);
...
function normalizeEditorContent() {
  els.visualEditor.querySelectorAll("a[href]").forEach(anchor => { anchor.target = "_blank"; anchor.rel = "noreferrer"; });
  els.visualEditor.querySelectorAll("figure:has(img), figure.image-block").forEach(figure => figure.classList.add("image-block"));
  els.visualEditor.querySelectorAll("img").forEach(img => {
  ...
  els.visualEditor.querySelectorAll(".code-block, .docsy-code-block").forEach(block => {
...
function renderSlashMenu() {
  els.slashMenu.innerHTML = slashMatches.map((block, index) => `<button class="slash-item ${index === activeSlashIndex ? "active" : ""}" type="button" data-index="${index}" role="option"><i class="${iconClass(block)}"></i><span><strong>${block.command} · ${escapeHtml(block.name)}</strong><span>${escapeHtml(block.description)}</span></span></button>`).join("");
...
function hideSlashMenu() { els.slashMenu.classList.remove("open"); slashMatches = []; slashRange = null; }
...
els.visualEditor.addEventListener("input", event => { autoLinkCurrentText(event); updateSlashMenu(); normalizeEditorContent(); updateEditorPlaceholder(); saveProject(); });
...
els.visualEditor.addEventListener("mouseup", saveSelection);
els.visualEditor.addEventListener("focus", saveSelection);
...
els.cancelImageBtn.addEventListener("click", () => els.imagePanel.classList.remove("open"));
els.insertImageBtn.addEventListener("click", insertImageFromPanel);
els.imagePanel.addEventListener("click", event => { if (event.target === els.imagePanel) els.imagePanel.classList.remove("open"); });
els.cancelTableBtn.addEventListener("click", () => els.tablePanel.classList.remove("open"));
els.insertTableBtn.addEventListener("click", insertTableFromPanel);
els.tablePanel.addEventListener("click", event => { if (event.target === els.tablePanel) els.tablePanel.classList.remove("open"); });
document.addEventListener("keydown", event => {
  const active = document.activeElement;
  if (els.slashMenu.classList.contains("open") && (els.visualEditor.contains(active) || els.slashMenu.contains(active))) {
    if (handleSlashMenuKey(event)) return;
  }
